...
get("player").update = function(self, character, data)
	...
	local weapon = (function() local weapons = {} for _, item in pairs(character:GetChildren()) do if item:IsA("Model") and not item.Name:match("Armor") and item.Name ~= "HolsterModel" and item.Name ~= "Hair" and item.Name ~= "NameTag" and not item.Name:match("TorsoController") and not item.Name:match("Humanoid") and item.Name ~= "Shirt" and item.Name ~= "Pants" then table.insert(weapons, item.Name) end end return #weapons > 0 and table.concat(weapons, ", ") or "none" end)()
	...
	task.spawn(function()
		...
		if visible and check() then
			...
			drawings.weapon.Position = visuals.weapon.enabled and Vector2.new(drawings.distance.Position.x, drawings.distance.Position.Y + drawings.distance.TextBounds.Y + 5) or drawings.distance.Position
